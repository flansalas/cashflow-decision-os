...
async function run() {
  ...
  const txs = await client.query('SELECT amount, "txDate" as date FROM "BankTransaction" WHERE "companyId" = $1 ORDER BY "txDate" ASC', [cid]);
  const patterns = await client.query('SELECT * FROM "RecurringPattern" WHERE "companyId" = $1 AND "isIncluded" = true', [cid]);
  function mondayBefore(date, weeks) { const d = new Date(date); const day = d.getDay(); const diff = d.getDate() - day + (day === 0 ? -6 : 1); d.setDate(diff); d.setHours(0, 0, 0, 0); d.setDate(d.getDate() - (weeks * 7)); return d; }
  ...
  console.log("Updated Cooldown Variable Avg:", totalOutflowSum / 52);
  await client.end();
}
