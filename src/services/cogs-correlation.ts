export interface COGSCorrelation {
    cashMarginRatio: number;      // e.g., 0.30 means 30¢ kept, 70¢ spent
    cogsLagWeeks: number;         // e.g., 2 means costs trail revenue by 2 weeks
    confidence: "high" | "med" | "low";
    weeksAnalyzed: number;
}

export function computeCOGSCorrelation(
    weeklyBuckets: Array<{ inflow: number; outflow: number }>
): COGSCorrelation {
    let activeWeeks = 0;
    ...
    for (const b of weeklyBuckets) {
        if (b.inflow > 0 || b.outflow > 0) activeWeeks++;
