...
const mockPrisma = vi.hoisted(() => ({
    receivableInvoice: {
        ...
        create: vi.fn().mockResolvedValue({ id: 'new_ar_1' }),
        update: vi.fn(),
...
describe('AR Upload Route - AI Isolation', () => {
    beforeEach(() => {
        vi.clearAllMocks();
    });
    ...
    it('returns success for accounting import even if AI scheduling and execution completely fails', async () => {
        const req = new NextRequest('http://localhost/api/upload/ar', {
            ...
            body: JSON.stringify({
                rows: [
                    { customerName: 'Test Corp', invoiceNo: 'INV-001', amountOpen: 100, status: 'open' }
                ],
                mappingJson: {}
            })
        });
        ...
        expect(mockPrisma.receivableInvoice.create).toHaveBeenCalled();
