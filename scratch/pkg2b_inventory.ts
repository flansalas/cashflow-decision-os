    ...
    try {
        await prisma.$transaction(async (tx) => {
            ...
            console.log("Latest AR Batch:", latestARBatch ? {
                id: latestARBatch.id, status: latestARBatch.status, createdAt: latestARBatch.createdAt
            } : "NONE");
            ...
            console.log("Latest AP Batch:", latestAPBatch ? {
                id: latestAPBatch.id, status: latestAPBatch.status, createdAt: latestAPBatch.createdAt
            } : "NONE");
            ...
            const activePatterns = await tx.recurringPattern.findMany({ where: { companyId, isActive: true } });
            console.log(`Active Patterns Count: ${activePatterns.length}`);
            const cadences = {};
            for (const p of activePatterns) {
                const key = `${p.cadence}_${p.category}`;
                cadences[key] = (cadences[key] || 0) + Number(p.amount);
            }
            ...
            console.log("Latest Baseline:", latestBaseline ? {
                ...
                weeklyInflow: latestBaseline.weeklyInflowAmount,
                weeklyOutflow: latestBaseline.weeklyOutflowAmount,
                confidence: latestBaseline.confidence,
                historyRecords: latestBaseline.BaselineSnapshotHistory.length
            } : "NONE");
            ...
            latestCheckpoints.forEach((cp, i) => {
                console.log(`Checkpoint ${i+1}: id=${cp.id}, sealed=${cp.isSealed}, versionHash=${cp.forecastVersionHash}, weeks=${cp.ForecastWeek.length}, plans=${cp.ExecutionPlan.length}`);
            });
            ...
            const totalRows = await tx.stagedImportRow.count({ where: { importBatch: { companyId } } });
            const appliedRows = await tx.stagedImportRow.count({ where: { importBatch: { companyId }, applyStatus: 'applied' } });
