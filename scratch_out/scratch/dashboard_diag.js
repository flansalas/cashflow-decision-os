...
const variance_1 = require("../src/services/variance");
const cogs_correlation_1 = require("../src/services/cogs-correlation");
const prisma_1 = __importDefault(require("../src/db/prisma"));
async function main() {
    const cid = "1a7b36f5-8fe0-4c2b-9336-8420846270b5";
    const [cashSnapshot, cashAdjustments, invoicesRaw, billsRaw, customerProfiles, vendorProfiles, assumptionRaw, recurringPatternsRaw, overrides, bankTxs, companyNotes, cashFlowCategories, cashFlowEntries, varianceLedger, customerPaymentObs,] = await Promise.all([
        prisma_1.default.cashSnapshot.findFirst({ where: { companyId: cid }, orderBy: [{ asOfDate: "desc" }, { createdAt: "desc" }] }),
        prisma_1.default.cashAdjustment.findMany({ where: { companyId: cid } }),
        prisma_1.default.receivableInvoice.findMany({ where: { companyId: cid } }),
        prisma_1.default.payableBill.findMany({ where: { companyId: cid } }),
        prisma_1.default.customerProfile.findMany({ where: { companyId: cid } }),
        prisma_1.default.vendorProfile.findMany({ where: { companyId: cid } }),
        prisma_1.default.assumption.findFirst({ where: { companyId: cid } }),
        prisma_1.default.recurringPattern.findMany({ where: { companyId: cid, status: "active" } }),
        prisma_1.default.override.findMany({ where: { companyId: cid, status: "active" }, orderBy: { createdAt: "desc" } }),
        prisma_1.default.bankTransaction.findMany({
            where: { companyId: cid },
            select: { amount: true, txDate: true, description: true, direction: true },
        }),
        prisma_1.default.companyNote.findMany({ where: { companyId: cid } }),
        prisma_1.default.cashFlowCategory.findMany({ where: { companyId: cid }, orderBy: [{ direction: "asc" }, { sortOrder: "asc" }, { name: "asc" }] }),
        prisma_1.default.cashFlowEntry.findMany({ where: { companyId: cid }, include: { category: true } }),
        prisma_1.default.baselineVarianceLedger.findMany({
            where: { companyId: cid },
            orderBy: { weekStart: "desc" },
            take: 8,
        }),
        prisma_1.default.customerPaymentObservation.findMany({
            where: { companyId: cid },
    ...
    const cachedBaseline = await prisma_1.default.baselineSnapshot.findUnique({
        where: { companyId: cid }
    });
    let baseline;
    if (cachedBaseline) {
    ...
    const cogsCorrelation = (0, cogs_correlation_1.computeCOGSCorrelation)(baseline.weeklyBuckets);
    const multipliers = (0, variance_1.computeVarianceMultipliers)(varianceLedger);
    ...
    const obsByCustomer = new Map();
    for (const obs of customerPaymentObs) {
        if (!obsByCustomer.has(obs.customerName))
            obsByCustomer.set(obs.customerName, []);
        obsByCustomer.get(obs.customerName).push(obs);
    }
    const overridesByTarget = new Map();
    for (const ov of overrides) {
        if (ov.targetId) {
            if (!overridesByTarget.has(ov.targetId))
                overridesByTarget.set(ov.targetId, []);
    ...
    const invoices = invoicesRaw.map(inv => {
        ...
        let overrideExpectedDate = null;
        let overrideAmount = null;
        let partialPayment = null;
        let isExcluded = false;
        for (const ov of ovs) {
            if (ov.type === "mark_paid")
                markedPaid = true;
            if (ov.type === "exclude")
                isExcluded = true;
            if (ov.type === "set_expected_payment_date" && ov.effectiveDate)
                overrideExpectedDate = ov.effectiveDate;
            if (ov.type === "adjust_amount" && ov.amount != null)
                overrideAmount = ov.amount;
            if (ov.type === "partial_payment" && ov.amount != null)
                partialPayment = ov.amount;
        }
        if (isExcluded)
            return null;
        return {
    ...
    const bills = billsRaw.map(bill => {
        ...
        for (const ov of ovs) {
            ...
            if (ov.type === "exclude")
                isExcluded = true;
            if (ov.type === "delay_due_date" && ov.effectiveDate)
                overrideDueDate = ov.effectiveDate;
            if (ov.type === "set_bill_due_date" && ov.effectiveDate)
                overrideDueDate = ov.effectiveDate;
            if (ov.type === "adjust_amount" && ov.amount != null)
                overrideAmount = ov.amount;
        }
    ...
    const skipDatesByPattern = new Map();
    for (const ov of overrides) {
        if ((ov.type === "skip_recurring_occurrence" || ov.type === "modify_recurring_occurrence") && ov.targetId && ov.effectiveDate) {
            if (!skipDatesByPattern.has(ov.targetId))
                skipDatesByPattern.set(ov.targetId, []);
    ...
    const forecast = (0, forecast_1.computeForecast)(forecastInput);
