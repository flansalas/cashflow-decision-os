...
function computeBaseline(txs, patterns, asOfDate, assumptions) {
    if (txs.length === 0) {
        return placeholderBaseline("No bank transactions available");
    }
    const { weekBuckets, dailyInflowSeries, dailyOutflowSeries } = (0, baseline_shared_1.prepareBaselineTransactions)(txs, patterns, asOfDate, assumptions, WEEKS_TO_ANALYZE);
    ...
    if (activeWeeks.length < exports.MIN_WEEKS_REQUIRED) {
        ...
        if (daySpan >= 1) {
            ...
            for (const p of excludedPatterns) {
                ...
                if (p.direction === "inflow")
                    recurringInflowTotal += (p.minAmount ?? p.typicalAmount) * occurrences;
                else
                    recurringOutflowTotal += (p.minAmount ?? p.typicalAmount) * occurrences;
            }
        ...
        return placeholderBaseline(`Only ${activeWeeks.length} active weeks found in 52-week history`);
    }
    const inflowValues = [];
    const outflowValues = [];
    const weights = [];
    ...
    for (let i = 0; i < WEEKS_TO_ANALYZE; i++) {
        ...
        if (firstActiveIdx !== -1 && (i < firstActiveIdx || i > lastActiveIdx))
            continue;
        const b = weekBuckets[i];
        const ageWeeks = (WEEKS_TO_ANALYZE - 1) - i;
        let weight = 1.0;
        if (ageWeeks <= 3)
            weight = 2.0; // Weeks 1-4:   highest weight
        else if (ageWeeks <= 7)
            weight = 1.5; // Weeks 5-8:   high weight
        else if (ageWeeks <= 12)
            weight = 1.0; // Weeks 9-12:  medium weight
        else if (ageWeeks <= 25)
            weight = 0.7; // Weeks 13-26: lower weight
        else
            weight = 0.4; // Weeks 27-52: background context
    ...
    let inflowCadence = undefined;
    let outflowCadence = undefined;
    if (activeWeeks.length >= 4) {
        // max lag 40 days to catch monthly
        const acfInflow = (0, acf_1.computeACF)(dailyInflowSeries, 40);
        // use lower threshold for residual noise
        const dominantInflowLag = (0, acf_1.detectDominantCadence)(acfInflow, 0.35);
        ...
        const acfOutflow = (0, acf_1.computeACF)(dailyOutflowSeries, 40);
        const dominantOutflowLag = (0, acf_1.detectDominantCadence)(acfOutflow, 0.35);
...
function trimmedValues(values, trimPct = 0.1) {
    if (values.length < 5)
