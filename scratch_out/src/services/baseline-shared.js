...
exports.mondayBefore = mondayBefore;
exports.addWeeks = addWeeks;
exports.addDays = addDays;
exports.daysBetween = daysBetween;
exports.prepareBaselineTransactions = prepareBaselineTransactions;
...
function prepareBaselineTransactions(txs, patterns, asOfDate, assumptions, weeksToAnalyze) {
    ...
    for (let i = 0; i < weeksToAnalyze; i++) {
        ...
        for (const tx of txs) {
            ...
            const txCategory = (0, detectPatterns_1.categorize)(tx.merchantKey || "");
            let matchesAssumption = false;
            if (assumptions) {
                if (assumptions.payrollAllInAmount &&
                ...
                if (!matchesAssumption &&
                    ...
                    const diff = Math.min(Math.abs(txDay - rentDay), Math.abs(txDay + 30 - rentDay), Math.abs(rentDay + 30 - txDay));
                    if (diff <= 3) {
                        matchesAssumption = true;
                    }
                }
            ...
            const matchedPattern = excludedPatterns.find(p => {
                return (0, detectPatterns_1.isRecurringIdentityMatch)({ description: tx.merchantKey, direction: txDirection, amount: absAmount, txDate: tx.date }, p, p.lastMatchedDate, p.cadence);
            });
            if (matchedPattern) {
            ...
            if (tx.amount > 0) {
                inflowSum += tx.amount;
                if (dayIndex >= 0 && dayIndex < dailyInflowSeries.length)
                    dailyInflowSeries[dayIndex] += tx.amount;
            }
            else {
                outflowSum += Math.abs(tx.amount);
                if (dayIndex >= 0 && dayIndex < dailyOutflowSeries.length)
                    dailyOutflowSeries[dayIndex] += Math.abs(tx.amount);
            }
        }
